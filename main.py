...
documents = Load_Data("Data").Load()
print(documents[0])

texts= Chunking(documents).Chunk_data()

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
embed_model = HuggingFaceEmbedding(model_name="BAAI/bge-small-en-v1.5", device=device)
texts_embedding = Convert(embed_model).model_embedd()
nodes = texts_embedding.get_nodes_from_documents(documents)
for node in nodes:
    print(node.get_text())
